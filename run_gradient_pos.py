...
class emulator(borg.forward.BaseForwardModel): 
    ...
    def adjointModel_v2_impl(self, input_ag):
        ...
        if self.use_cic:
            ...
            self.ag_pos1 = np.transpose(np.asarray(self.jax_cic_grad(self.ag_dens)))
            self.ag_pos2 = np.asarray(cic_analytical_grad(self.ag_dens,self.pos_temp[:,0],self.pos_temp[:,1],self.pos_temp[:,2],128,128,128,250,250,250))
